...
public class Main {
    public static void main(String[] args) {
        ...
        phoneFactory1.produce("아이폰 17");    //재정의 된 메서드 # 1- 추상메서드
        phoneFactory1.manage();                     // 재정의 된 메서드 # 2- 추상메서드
        phoneFactory1.diplayInfo();                 // 부모 메서드 사용
        ...
        factory1.setName("가전 제품 공장");           //이건 메서드 호출에 해당하죠
        factory1.produce("백색 가전");          // 얘도
        /*
            이상의 개념은 추상 클래스의 개념에서 벗어납니다(객체 생성이 불가능하다고 했는데 Factory 클래스의 객체가 생성되었다는 점에서).
            Java17인지 11인지에서 도입된 익명 클래스라는 개념인데, 재사용하지 않고 한 번만 쓰고 치울거라면(즉, 이 Main 클래스에서만 사용하고 다른 클래스에서는
